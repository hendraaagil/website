...
updatedAt: 2024-11-15T10:10:00+07:00
---
...
Di kesempatan kali ini saya ingin coba untuk deploy ke VPS, tapi dengan menggunakan Docker.

---

## Kenapa Docker?

[Docker](https://www.docker.com/) dapat meng-_containerize_ suatu aplikasi / project kita sehingga semua setup hanya perlu dilakukan pada container tersebut, tidak perlu di VPS-nya secara langsung.
...
Lalu, setup SSH agent dan deploy ke VPS dengan melakukan pull image dan jalankan container dengan image terbaru. Jangan lupa untuk generate private key (`VPS_PRIVATE_KEY`) terlebih dahulu jika belum ada. Isi `VPS_USER` dengan username yang terdaftar di server dan `VPS_HOST` dengan IP address dari server yang digunakan.
...
Kode lengkap workflow-nya kurang lebih seperti berikut:
...
Untuk HTTP server-nya sendiri saya menggunakan [Fastify](https://fastify.dev/) dan ketika deploy pertama kali, server mengembalikan empty response saat saya mengakses IP server, yang seharusnya mengembalikan sebuah JSON. Setelah saya [telusuri](https://stackoverflow.com/a/52525832/12078166) ternyata ketika kita menggunakan Docker, kita perlu bind aplikasi ke host `0.0.0.0`.
...
Itulah sedikit pengalaman saya ketika melakukan deployment dengan Docker dan GitHub Actions. Bagi saya yang belum terlalu tertarik untuk mendalami masalah deployment dan server ini ternyata cukup rumit, walau pada akhirnya tinggal push dan auto deploy serasa menggunakan Vercel 😋

Docker juga menyediakan [cheatsheet](https://docs.docker.com/get-started/docker_cheatsheet.pdf) untuk CLI-nya yang cukup membantu bagi orang yang belum cukup familiar dengan Docker seperti saya. Sekian tulisan kali ini, jika ada yang salah mohon diluruskan 🙏

Dan jika kalian ingin mencoba bot telegram yang sedang dalam development ini, bisa mengunjungi: [https://t.me/gilchatbot](https://t.me/gilchatbot) 🤘
